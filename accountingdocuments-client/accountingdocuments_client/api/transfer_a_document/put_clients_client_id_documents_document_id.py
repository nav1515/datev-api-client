...
from ...models.put_clients_client_id_documents_document_id_body import PutClientsClientIdDocumentsDocumentIdBody
...
def _get_kwargs(
    ...
    document_id: str,
    *,
    body: PutClientsClientIdDocumentsDocumentIdBody,
    ...
    _kwargs: dict[str, Any] = {
        "method": "put",
        "url": "/clients/{client_id}/documents/{document_id}".format(client_id=client_id,document_id=document_id,),
    }
...
def sync_detailed(
    ...
    *,
    ...
    r""" Transfers a document with an ID

     A new accounting document with the ID of the document in the format of a GUID will be transferred to
    Belege online in DATEV Unternehmen online.

    **Please note:** If the ID of the document already exists in Belege online, then the import will be
    aborted with an error.

    Each accounting document is transferred in a multipart object as a single file with optional
    metadata (document type and note). The content-type indicated in the multipart object has to
    correspond to the file. The accepted content-types and the accepted file size are determined by
