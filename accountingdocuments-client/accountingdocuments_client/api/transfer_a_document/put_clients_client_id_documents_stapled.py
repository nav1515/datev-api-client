...
from ...models.put_clients_client_id_documents_stapled_body import PutClientsClientIdDocumentsStapledBody
...
def _get_kwargs(
    client_id: str,
    *,
    body: PutClientsClientIdDocumentsStapledBody,
    ...
    _kwargs: dict[str, Any] = {
        "method": "put",
        "url": "/clients/{client_id}/documents/stapled".format(client_id=client_id,),
    }
...
def sync_detailed(
    ...
    *,
    ...
    r""" Transfers multiple documents

     New accounting documents will be transferred to Belege online in DATEV Unternehmen online. The
    documents become stapled in the incoming order.

    Each accounting document is transferred in a multipart object as a single file. Only one optional
    metadata (document type and note) is allowed. The content-type indicated in the multipart object has
    to correspond to the file. The accepted content-types and the accepted file size are determined by
    Belege online. Exception: ZIP files are not supported.

    It is recommended that the user chooses one document type for all accounting documents, as the
    ...
    If the value of document_type is empty, then the parameter will be ignored and will not be returned
    in the response. In this case, and if the parameter document_type is not used, the term “ohne
    Belegtyp” (without document type) is displayed in the DATEV applications.
