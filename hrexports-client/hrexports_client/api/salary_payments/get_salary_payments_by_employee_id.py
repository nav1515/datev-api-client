...
def _get_kwargs(
    ...
    *,
    ...
    _kwargs: dict[str, Any] = {
        "method": "get",
        "url": "/clients/{client_id}/employees/{employee_id}/salarypayments".format(client_id=client_id,employee_id=employee_id,),
        "params": params,
    }
...
def _parse_response(*, client: Union[AuthenticatedClient, Client], response: httpx.Response) -> Optional[Union[ErrorMessage, ErrorMessage5Xx, SalaryPayments]]:
    if response.status_code == 200:
        response_200 = SalaryPayments.from_dict(response.json())
...
def _build_response(*, client: Union[AuthenticatedClient, Client], response: httpx.Response) -> Response[Union[ErrorMessage, ErrorMessage5Xx, SalaryPayments]]:
    return Response(
...
) -> Response[Union[ErrorMessage, ErrorMessage5Xx, SalaryPayments]]:
    """ Get the employee's salary payments

     Salary components (Lohnarten, Nettobezüge, Nettoabzüge) of a an employee can be accessed by its
    employee-id.
...
def sync(
    ...
    *,
...
) -> Optional[Union[ErrorMessage, ErrorMessage5Xx, SalaryPayments]]:
    """ Get the employee's salary payments
    ...
    Returns:
        Union[ErrorMessage, ErrorMessage5Xx, SalaryPayments]
     """
