...
def _get_kwargs(
    ...
    *,
    ...
    _kwargs: dict[str, Any] = {
        "method": "get",
        "url": "/clients/{client_id}/employees/salarytotalvalues".format(client_id=client_id,),
        "params": params,
    }
...
def _parse_response(*, client: Union[AuthenticatedClient, Client], response: httpx.Response) -> Optional[Union[ErrorMessage, ErrorMessage5Xx, list['SalaryTotalValues']]]:
    if response.status_code == 200:
        response_200 = []
        _response_200 = response.json()
        for response_200_item_data in (_response_200):
            response_200_item = SalaryTotalValues.from_dict(response_200_item_data)
...
def _build_response(*, client: Union[AuthenticatedClient, Client], response: httpx.Response) -> Response[Union[ErrorMessage, ErrorMessage5Xx, list['SalaryTotalValues']]]:
    return Response(
...
) -> Response[Union[ErrorMessage, ErrorMessage5Xx, list['SalaryTotalValues']]]:
    """ Get the list of employee's salary total values

     Salary (Gesamtbrutto, Auszahlungsbetrag, Nettoverdienst) of all employees of an client can be
...
def sync(
    ...
    *,
...
) -> Optional[Union[ErrorMessage, ErrorMessage5Xx, list['SalaryTotalValues']]]:
    """ Get the list of employee's salary total values
    ...
    Returns:
        Union[ErrorMessage, ErrorMessage5Xx, list['SalaryTotalValues']]
     """
